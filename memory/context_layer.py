...
Integrates salience filter, rule-based compressor, token budget manager,
and incremental processor into a single cohesive system. Provides clean API
for Ada (short-term memory) and Nexus (long-term memory/graph) components.
"""
...
class SalienceFilter:
    ...
    def compute_salience(self, entry: MemoryEntry, current_context: str) -> float:
        ...
        # Simplified scoring - can be extended
        recency = 1.0 / (1.0 + (self._current_time() - entry.timestamp)) if entry.timestamp else 0.5
...
class TokenBudgetManager:
    ...
    def fit_to_budget(self, entries: List[MemoryEntry], current_context: str) -> Tuple[List[MemoryEntry], int]:
        """Select entries that fit within available budget."""
        selected = []
...
class MemoryContextLayer:
    ...
    def add_memory(self, content: str, memory_type: MemoryType, timestamp: float = None):
        """
    ...
    def assemble_context(self, current_query: str, memory_types: List[MemoryType] = None) -> ContextWindow:
        """
        ...
        if not candidates:
            return ContextWindow(text=current_query, token_count=self.token_manager.count_tokens(current_query), entries_included=[], budget_remaining=self.token_manager.available_for_memory)
        
        # Step 1: Filter by salience
        salient_entries = self.salience_filter.filter_top_k(candidates, self.top_k_salient, current_query)
        
        # Step 2: Compress entries
        compressed_entries = self.compressor.compress_batch(salient_entries)
        
        # Step 3: Fit to token budget
        selected_entries, remaining_budget = self.token_manager.fit_to_budget(compressed_entries, current_query)
